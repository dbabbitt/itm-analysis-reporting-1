#!/usr/bin/env python
# Utility Functions to build the Open World dataset.
...
# C:\Users\DaveBabbitt\anaconda3\python.exe build_ow_dataset.py
# To run this in Ubuntu, type something like:
# cd /mnt/c/Users/DaveBabbitt/Downloads; /home/dbabbitt/anaconda3/envs/itm_analysis_reporting/bin/python build_ow_dataset.py
...
std_dev = json_stats_df.std()

# Get the indices of columns with zero variance
zero_variance_cols = std_dev[std_dev == 0].index

# Remove columns with zero variance
column_count = json_stats_df.shape[1]
json_stats_df.drop(columns=zero_variance_cols, inplace=True)
if IS_DEBUG: print(f"Dropped {int(column_count - json_stats_df.shape[1])} JSON columns with zero variance")

# Assess the number of unique values (excluding NaNs) in each column
unique_values = json_stats_df.nunique(dropna=True)
...
column_count = json_stats_df.shape[1]
json_stats_df.drop(columns=few_unique_cols, inplace=True)
if IS_DEBUG: print(f"Dropped {int(column_count - json_stats_df.shape[1])} JSON columns with fewer than two unique values")
...
# data_frames_dict = nu.load_data_frames(
    # metrics_evaluation_open_world_json_stats_df=''
# )
# json_stats_df = data_frames_dict['metrics_evaluation_open_world_json_stats_df']

if 'logger_version' not in json_stats_df.columns:
    
    print("Get the columns that consistently have only one value in them per session")
...
if mask_series.any():
    print(json_stats_df[mask_series].dropna(axis='columns', how='all')); raise
...
if mask_series.any():
    ...
    for session_uuid, idx_df in json_stats_df[mask_series].groupby('session_uuid'):
        ...
        if mask_series.any():
            ...
            assert mask_series.any(), f"Session ID {session_uuid} doesn't have an event times in metrics_evaluation_open_world_csv_stats_df"
        ...
        else: print(f"You're missing a Session ID {session_uuid} in metrics_evaluation_open_world_csv_stats_df")
    nu.save_data_frames(metrics_evaluation_open_world_json_stats_df=json_stats_df, verbose=IS_DEBUG)
    mask_series = json_stats_df.session_file_date.isnull()
    if mask_series.any():
        print(f'I still have {mask_series.sum()} sessioms in my JSON Stats data frame without file dates.')
        print(json_stats_df[mask_series].dropna(axis='columns', how='all').T)
...
if (new_column_name not in json_stats_df.columns):
    ...
    # Store the results and show the new data frame shape
    nu.save_data_frames(metrics_evaluation_open_world_json_stats_df=json_stats_df, verbose=IS_DEBUG)
if IS_DEBUG: print(json_stats_df.groupby(new_column_name).size().to_frame().rename(columns={0: 'record_count'}))
...
for (session_uuid, scene_id), scene_df in csv_stats_df.groupby(fu.scene_groupby_columns):
    for env_str in encounter_layouts_list:
        ...
        if all(map(lambda patient_id: patient_id in scene_df.patient_id.unique().tolist(), patients_list)):
            mask_series = (json_stats_df.session_uuid == session_uuid)
            json_stats_df.loc[mask_series, new_column_name] = env_str
            nu.save_data_frames(metrics_evaluation_open_world_json_stats_df=json_stats_df, verbose=IS_DEBUG)
if IS_DEBUG: print(json_stats_df.groupby(new_column_name, dropna=False).size().to_frame().rename(columns={0: 'record_count'}))
...
intersection_columns = set([
    ...
    'player_location_right_hand_location', 'bag_access_location', 'patient_engaged_health_level', 'voice_capture_command_description', 'csv_file_name', 'csv_file_subpath'
])
...
if drop_columns:
    ...
    nu.save_data_frames(metrics_evaluation_open_world_scene_stats3318_df=scene_stats_df, verbose=True)
...
mask_series = scene_stats_df.duplicated(subset=fu.scene_groupby_columns)
if mask_series.any():
    if IS_DEBUG: print("\nRemove duplicates from the scene stats data frame")
    scene_stats_df = scene_stats_df[~mask_series]
    nu.save_data_frames(metrics_evaluation_open_world_scene_stats3333_df=scene_stats_df, verbose=True)

# 
# #print("Provide Correctly Grouped Responder Type Stats")
...
if (new_column_name not in json_stats_df.columns):
    ...
    nu.save_data_frames(metrics_evaluation_open_world_json_stats_df=json_stats_df, verbose=IS_DEBUG)
if IS_DEBUG: print(json_stats_df.groupby(new_column_name).size().to_frame().rename(columns={0: 'record_count'}).sort_values(
    'record_count', ascending=False
).head(5))
...
if any(map(lambda x: x not in json_stats_df.columns, groupby_columns)):
    ...
    file_path = osp.join(nu.data_folder, 'xlsx', 'Responder_Categories_and_Counts_DPW.xlsx')
    dpw_responder_categories_df = read_excel(file_path)

    print("Get description data frame")
    ...
    print("Get categories data frame")
    ...
    print("Create global description column")
    ...
    print("Create sub description column")
    ...
    print("Add columns to JSON Stats data frame")
    ...
    if 'responder_type' not in json_stats_df.columns:
        json_stats_df['responder_type'] = json_stats_df.MedRole.map(
            lambda x: sub('^Other$', 'Other HP', ' '.join([r.title() for r in get_value_description('MedRole', x).split(' ')]).replace('Em ', 'EM ').replace('EM Faculty', 'EM-Faculty'))
        )
...
if all(map(lambda x: x in json_stats_df.columns, groupby_columns)):
    ...
    print(json_stats_df.groupby(groupby_columns).size().to_frame().rename(columns={0: 'record_count'}).sort_values(
        'record_count', ascending=False
    ).head(5))

if IS_DEBUG: print('Spot check for csv_stats/scene_stats consistency')
df = read_csv(osp.join(nu.saves_csv_folder, 'metrics_evaluation_open_world_csv_stats_df.csv'))
all_uuids_list = sorted(df.session_uuid.unique())
df = read_csv(osp.join(nu.saves_csv_folder, 'metrics_evaluation_open_world_scene_stats_df.csv'))
scene_uuids_list = sorted(df.session_uuid.unique())
assert set(scene_uuids_list).symmetric_difference(set(all_uuids_list)) == set(), "Session IDs in the raw files are missing from the final scene stats"


# load data frames to get a reliable representation
if IS_DEBUG: print('\nCreate the final dataframe that we should use for analysis')
nu.save_data_frames(csv_stats_df=csv_stats_df, verbose=True)
nu.save_data_frames(json_stats_df=json_stats_df, verbose=True)
nu.save_data_frames(scene_stats_df=scene_stats_df, verbose=True)
...
if IS_DEBUG: print('\nThe JSON stats dataset columns we want to have in the merge:')
survey_columns = ['AD_KDMA_Sim', 'AD_KDMA_Text', 'PropTrust', 'ST_KDMA_Sim', 'ST_KDMA_Text', 'YrsMilExp']
senario_columns = sorted([cn for cn in json_stats_df.columns if 'scenarioData' in cn] + survey_columns)
if IS_DEBUG: print(senario_columns)

columns_list = on_columns + senario_columns
if set(columns_list).issubset(set(json_stats_df.columns)):
    merge1_df = csv_stats_df.merge(json_stats_df[columns_list], on=on_columns, how='left')
    nu.save_data_frames(merge1_df=merge1_df, verbose=True)
    print("\nColumns to merge the scene stats dataset with the previous merge on:")
...
columns_list = [cn for cn in ['participant_id'] + fu.scene_groupby_columns + ['medical_role', 'encounter_layout'] + mean_survey_columns + [
    'mean_configData_scenarioData_difficulty', 'mean_actual_engagement_distance', 'mean_first_engagement', 'mean_first_treatment', 'mean_injury_correctly_treated_count', 'mean_injury_not_treated_count',
    'mean_injury_treatments_count', 'mean_injury_wrongly_treated_count', 'mean_last_engagement', 'mean_last_still_engagement', 'mean_measure_of_right_ordering', 'mean_patient_count',
    'mean_percent_hemorrhage_controlled', 'mean_pulse_taken_count', 'mean_stills_value', 'mean_teleport_count', 'mean_time_to_hemorrhage_control_per_patient', 'mean_time_to_last_hemorrhage_controlled',
    'mean_total_actions_count', 'mean_triage_time', 'mean_voice_capture_count', 'mean_walk_command_count', 'mean_walk_value', 'mean_walkers_value', 'mean_wave_command_count', 'mean_wave_value'
] + metadata_columns if cn in anova_df.columns]
...
function_call_dict = {
    'encounter_layout': 'fu.add_encounter_layout_column_to_json_stats', 'medical_role': 'fu.add_medical_role_column_to_anova_dataframe',
    'mean_prioritize_high_injury_severity_patients': 'fu.add_prioritize_severity_column_to_merge_dataframe'
}
